...
public interface CardControllerDocs {
    ...
    ApiResponse<PresignedUrlResponseDTO> getPresignedPutUrlForNewCard(
            ...
            @Parameter(description = "그룹북 식별자(ID)", example = "1")
    ...
    @Operation(
            ...
            - **카드 소유자(GroupBook 소유자)만** 발급 가능합니다. 그룹 멤버는 카드 목록/상세 조회 시 응답에 포함된 presignedGetUrl로 이미지 보기만 가능합니다.
            - UUID 기반 s3Key를 생성하여 presigned URL을 발급합니다.
            - **카드 수정 시 이미지 변경에 사용합니다.** 카드 생성 전 이미지 업로드는 `POST /api/cards/{groupBookId}/presigned-url`을 사용하세요.
    ...
    @Operation(
            ...
            - 카드 소유자(GroupBook 소유자)이거나 같은 그룹의 멤버인 경우에만 조회 가능합니다.
    ...
    @Operation(
            ...
            - 카드 소유자(GroupBook 소유자)만 수정 가능합니다.

...
public interface CardStateRepository extends JpaRepository<CardState, Long> {
    ...
    @Query("SELECT cs FROM CardState cs JOIN FETCH cs.card c LEFT JOIN FETCH c.cardImage LEFT JOIN FETCH c.groupBook gb LEFT JOIN FETCH gb.group g LEFT JOIN FETCH g.book WHERE cs.user.id = :userId AND cs.bookmarked = true ORDER BY cs.createdAt DESC")

...
import com.example.bookiibookii.domain.groupbook.dto.CardImageUpdateResult;
...
import com.example.bookiibookii.domain.groupbook.dto.res.CardImageResponseDTO;
...
import com.example.bookiibookii.domain.groupbook.entity.CardState;
import com.example.bookiibookii.domain.groupbook.entity.GroupBook;
import com.example.bookiibookii.domain.groupbook.exception.CardException;
import com.example.bookiibookii.domain.groupbook.exception.CardImageException;
import com.example.bookiibookii.domain.groupbook.exception.code.CardErrorCode;
...
import com.example.bookiibookii.domain.groupbook.repository.CardImageRepository;
import com.example.bookiibookii.domain.groupbook.repository.CardRepository;
import com.example.bookiibookii.domain.groupbook.repository.CardStateRepository;
...
public class CardService {
    ...
    /**
     * Presigned PUT URL 발급. GroupBook 존재 및 소유권 검증 후 DTO 반환.
     */
    public PresignedUrlResponseDTO getPresignedPutUrlForNewCard(Long groupBookId, Long userId, int presignedPutUrlExpirationMinutes) {
        groupBookRepository.findByIdAndUser_Id(groupBookId, userId)
    ...
    /**
     * 카드 이미지 수정용 Presigned PUT URL 발급. 카드 소유자(GroupBook 소유자)만 발급 가능.
     */
     ...
     */
    @Transactional
    public CardCreateResponseDTO createCard(Long groupBookId, Long userId,
        ...
        if (page != null) {
            Integer totalPages = groupBook.getBook().getTotalPages();
    ...
    public CardListResponseDTO getCardsByGroupId(Long groupId, Long userId, int presignedGetUrlExpirationMinutes) {
        ...
        List<Card> cards = cardRepository.findByGroup_GroupIdWithCardImageAndGroupBookAndGroupAndBook(groupId);
        ...
        if (group.getGroupType() == GroupType.RELAY) {
            myComment = groupBookRepository.findByUser_IdAndGroup_GroupId(userId, groupId)
            ...
            if (currentOwnerOpt.isPresent()
                ...
                partnerComment = groupBookRepository.findByUser_IdAndGroup_GroupId(partnerUserId, groupId)
                        .map(ub -> ub.getComment())
                        .orElse(null);
            }
        }
    ...
    /**
     * 카드 소유자(GroupBook 소유자)만 접근 가능한 조회. 카드 이미지 수정용 Presigned URL 발급 등에서 사용.
     */
    public Card getCardForOwner(Long cardId, Long userId) {
        Card card = cardRepository.findByIdWithCardImageAndGroupBookAndBook(cardId)
                .orElseThrow(() -> new CardImageException(CardImageErrorCode.CARD_NOT_FOUND));

        if (!card.getGroupBook().getUser().getId().equals(userId)) {
            throw new CardImageException(CardImageErrorCode.CARD_NOT_FOUND);
        }
    ...
    private Card updateCard(Long cardId, Long userId, Integer page, String memo, String s3Key) {
        Card card = cardRepository.findByIdWithGroupBookAndGroup(cardId)
                .orElseThrow(() -> new CardImageException(CardImageErrorCode.CARD_NOT_FOUND));


        if (page != null) {
            Integer totalPages = card.getGroupBook().getBook().getTotalPages();
            validatePage(page, totalPages);
    ...
    private Card getCardWithCardImageAndBook(Long cardId) {
        return cardRepository.findByIdWithCardImageAndGroupBookAndBook(cardId)
                .orElseThrow(() -> new CardImageException(CardImageErrorCode.CARD_NOT_FOUND));
    }
    ...
    private Card getCardDetail(Long cardId, Long userId) {
        ...
        boolean isOwner = card.getGroupBook().getUser().getId().equals(userId);
    ...
    private String getCreatorNameSafely(Card card) {
        if (card == null || card.getGroupBook() == null || card.getGroupBook().getUser() == null) {
            return "";
        }
        String name = card.getGroupBook().getUser().getNickName();
    ...
    private String getBookTitleSafely(Card card) {
        if (card == null || card.getGroupBook() == null) {
            return "";
        }
        var group = card.getGroupBook().getGroup();
    ...
    private int calculateUserReadingRate(Long userId, Groups group) {
        return cardRepository.findTopByGroupBook_User_IdAndGroupOrderByPageDesc(userId, group)
                .map(card -> {
                    GroupBook ub = card.getGroupBook();
